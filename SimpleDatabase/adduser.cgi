...
if 'HTTP_COOKIE' in os.environ:
	...
	if row == None or pw != row[2]:
		...
		if role == "Owner":
			...
			pw = form.getvalue('pw')
			query = 'select Name from Users where Name like "' + account + '"'
			cursor.execute(query)
			row = cursor.fetchone()
			if row == None:
				insert = 'insert into Users(Name, Role, Password) values("' + account + '", "Visitor", "' + pw + '")'
				cursor.execute(insert)
				db.commit()
				print
				print REDIRECT % {'MESSAGE': "User account sucessfully added!"}
			else:
				print
				print REDIRECT % {'MESSAGE': "User name already exists!"}
		elif role == "Visitor":
			print "content-type: text/html"
